...
/**
 * Exception class for handling file system-related errors.
 * When this exception is thrown, an error message is created and will be processed by an HttpResponse object.
 */
public class FileSystemException extends Exception implements ErrorMessageHandler {

    private final HttpMessage errorMessage;

    /**
     * Constructs a new FileSystemException with the specified error message and HTTP response status.
     *
     ...
     */
    public FileSystemException(String message, HttpResponseStatus httpResponseStatus) {
        super(message);
        this.errorMessage = createErrorMessage(message, httpResponseStatus);
    }

    /**
     * Constructs a new FileSystemException with the specified error message, HTTP response status, and cause.
     *
     * @param message The error message explaining the reason for the exception.
     * @param httpResponseStatus The HTTP response status (e.g., 404, 500) associated with the error.
     * @param cause The cause of the exception.
     */
    public FileSystemException(String message, HttpResponseStatus httpResponseStatus, Throwable cause) {
        super(message, cause);
        this.errorMessage = createErrorMessage(message, httpResponseStatus);
    }
