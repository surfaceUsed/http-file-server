...
public class HttpResponse {
    ...
    void initResponse(HttpMessage message, byte[] responseBody) {
        ...
        setStatusLine(HttpResponseInitializer.createResponseLine(HTTP_VERSION, message.getServerResponse()));
    ...
    public void sendResponse(OutputStream output) throws IOException {
        ...
        //LOGGER.info(loggResponse());
        output.flush();
    }

    /*
    // Vet ikke om det er behov for dette.....
    private String loggResponse() {
        StringBuilder sb = new StringBuilder(this.statusLine + HttpResponseInitializer.getHeadersAsString(this.headers));
        return (!getBody().isEmpty()) ? sb.append(getBody()).toString() : sb.toString();
    }

    // TRenger ikke senere.
    private String getBody() {
        StringBuilder sb = new StringBuilder();
        if (this.getServerResponseContentType() != ContentType.APPLICATION_OCTET_STREAM) {
            for (byte b : this.responseBody) {
                sb.append((char) b);
            }
        }
        return sb.toString();
